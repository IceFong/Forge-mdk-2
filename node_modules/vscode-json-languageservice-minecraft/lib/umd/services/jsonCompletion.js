(function (factory) {
    if (typeof module === "object" && typeof module.exports === "object") {
        ...
        define(["require", "exports", "../parser/jsonParser", "jsonc-parser", "../utils/json", "../utils/strings", "../utils/objects", "vscode-languageserver-types", "../minecraftServices/minecraftCompletion", "vscode-nls"], factory);
    }
    ...
    var minecraftCompletion_1 = require("../minecraftServices/minecraftCompletion");
    ...
    var JSONCompletion = /** @class */ (function () {
        function JSONCompletion(schemaService, contributions, promiseConstructor, clientCapabilities) {
            ...
            this.minecraftCompletion = new minecraftCompletion_1.MinecraftCompletion();
                        ...
                        if (node.type === 'array' && s.schema.items) {
                            if (Array.isArray(s.schema.items)) {
                                ...
                                if (index < s.schema.items.length) {
                                    _this.addSchemaValueCompletions(doc, s.schema.items[index], separatorAfter_2, collector, types);
                                }
                            }
                            else {
                                _this.addSchemaValueCompletions(doc, s.schema.items, separatorAfter_2, collector, types);
                            }
                        }
                        if (s.schema.properties) {
                            var propertySchema = s.schema.properties[parentKey];
                            if (propertySchema) {
                                _this.addSchemaValueCompletions(doc, propertySchema, separatorAfter_2, collector, types);
                            }
                        }
                    }
                });
        ...
        JSONCompletion.prototype.addSchemaValueCompletions = function (doc, schema, separatorAfter, collector, types) {
            var _this = this;
            if (typeof schema === 'object') {
                ...
                if (Array.isArray(schema.allOf)) {
                    schema.allOf.forEach(function (s) { return _this.addSchemaValueCompletions(doc, s, separatorAfter, collector, types); });
                }
                if (Array.isArray(schema.anyOf)) {
                    schema.anyOf.forEach(function (s) { return _this.addSchemaValueCompletions(doc, s, separatorAfter, collector, types); });
                }
                if (Array.isArray(schema.oneOf)) {
                    schema.oneOf.forEach(function (s) { return _this.addSchemaValueCompletions(doc, s, separatorAfter, collector, types); });
                }
            }
        };
